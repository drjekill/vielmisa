...
class Calendario_model extends CI_MODEL
{
    ...
    public function generar_calendario($year, $month) {
        ...
        $conf_calendar = array('show_next_prev' => TRUE, 'next_prev_url' => 'http://localhost/calendario_ci/calendario/cal', 'start_day' => 'lunes', 'template' => '  
               ...
               {table_open}<table border="0" cellpadding="0" cellspacing="0" class="table-responsive calendario">{/table_open}
     ...
     * @param $month - int número de mes
     * @param $year - int número de añocitas
     */
    public function insert_horas($year, $month, $day, $espacio_id) 
    {
        ...
        if ($query->num_rows() < 1) {
            ...
            $sql ='SELECT
                    ...
                    FROM
                       espacios
                        INNER JOIN horas_espacio 
                            ON (espacios.id = horas_espacio.espacio_id)
                        INNER JOIN horas 
                            ON (horas.id = horas_espacio.hora_id)
     ...
     * @param $year - int número de año
     */
    public function horas_seleccionadas($year, $month, $day, $espacio_id) 
    {
     ...
     * @param $year - int número de año
     */
    public function nueva_cita($dia_calendario, $hora, $comentario_cita, $estado ,$espacio_id) 
    {
